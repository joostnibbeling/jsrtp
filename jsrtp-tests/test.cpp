...
TEST(KEY_DERIVATION, AES_CM_SALT)
{
	std::vector<uint8_t> master_key = { 0xE1,0xF9,0x7A,0x0D,0x3E,0x01,0x8B,0xE0,0xD6,0x4F,0xA3,0x2C,0x06,0xDE,0x41,0x39 };
	std::vector<uint8_t> master_salt = { 0x0E,0xC6,0x75,0xAD,0x49,0x8A,0xFE,0xEB,0xB6,0x96,0x0B,0x3A,0xAB,0xE6 };
	std::vector<uint8_t> key_e{ 0x30,0xCB,0xBC,0x08,0x86,0x3D,0x8C,0x85,0xD4,0x9D,0xB3,0x4A,0x9A,0xE1 };
	uint64_t n = 14;
	uint8_t label = 0x2;
...
TEST(KEY_DERIVATION, AES_CM_AUTH)
{
	std::vector<uint8_t> master_key = { 0xE1,0xF9,0x7A,0x0D,0x3E,0x01,0x8B,0xE0,0xD6,0x4F,0xA3,0x2C,0x06,0xDE,0x41,0x39 };
	std::vector<uint8_t> master_salt = { 0x0E,0xC6,0x75,0xAD,0x49,0x8A,0xFE,0xEB,0xB6,0x96,0x0B,0x3A,0xAB,0xE6 };
	std::vector<uint8_t> key_e{ 0xCE, 0xBE, 0x32, 0x1F, 0x6F, 0xF7, 0x71, 0x6B, 0x6F, 0xD4, 0xAB, 0x49, 0xAF, 0x25, 0x6A, 0x15, 0x6D, 0x38, 0xBA, 0xA4, 0x8F, 0x0A,
