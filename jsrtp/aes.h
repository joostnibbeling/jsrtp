...
public:
	...
	void encrypt(Container& plain_text) const;

	template<typename Iter>
	void encrypt(Iter plain_text, int size) const;

	template<typename Container>
	void decrypt(Container& cipher_text) const;

	template<typename Iter>
	void decrypt(Iter cipher_text, int size) const;
	...
	private:
		...
		template<typename Iter_out, typename Iter_in1, typename Iter_in2>
...
private:
	...
	template<typename Iter>
	void encrypt_block(Iter block) const;
	
	template<typename Iter>
	void add_key(Iter block, ByteVectorConstIt key) const;

	template<typename Iter>
	void sub_bytes(Iter block) const;

	template<typename Iter>
	void shift_rows(Iter block) const;

	template<typename Iter>
	void mix_columns(Iter block) const;

	template<typename Iter>
	void decrypt_block(Iter block) const;

	template<typename Iter>
	void inverse_sub_bytes(Iter block) const;

	template<typename Iter>
	void inverse_shift_rows(Iter block) const;

	template<typename Iter>
	void inverse_mix_columns(Iter block) const;
	...
	static uint8_t sbox_get_column(uint8_t in);
	static uint8_t sbox_get_row(uint8_t in);
};

template<typename Container>
void AES::encrypt(Container& plain_text) const
{
	if (plain_text.size() % BLOCK_SIZE != 0)
	{
		throw std::invalid_argument("AES - Encrypt input not multiple of block size");
	}
...
template<typename Iter>
void AES::encrypt(Iter plain_text, int size) const
{
	...
	for (int block_ind = 0; block_ind < blocks; ++block_ind)
	{
		auto block = plain_text + block_ind * BLOCK_SIZE;
		encrypt_block(block);
	}
}

template<typename Container>
void AES::decrypt(Container& cipher_text) const
{
	if (cipher_text.size() % BLOCK_SIZE != 0)
	{
		throw std::invalid_argument("AES - Decrypt input not multiple of block size");
	}
...
template<typename Iter>
void AES::decrypt(Iter cipher_text, int size) const
{
	if (size % BLOCK_SIZE != 0)
	{
		throw std::invalid_argument("AES - Decrypt input not multiple of block size");
	...
	for (int block_ind = 0; block_ind < blocks; ++block_ind)
	{
		auto block = cipher_text + block_ind * BLOCK_SIZE;
		decrypt_block(block);
	}
}

template<typename Iter>
void AES::encrypt_block(Iter block) const
{
...
template<typename Iter>
void AES::add_key(Iter block, ByteVectorConstIt key) const
{
...
template<typename Iter>
void AES::sub_bytes(Iter block) const
{
	for (int i = 0; i < BLOCK_SIZE; i++)
	{
...
template<typename Iter>
void AES::shift_rows(Iter block) const
{
...
template<typename Iter>
void AES::mix_columns(Iter block) const
{
	state mixed;
	for (int i = 0; i < 4; i++)
	{
...
template<typename Iter>
void AES::decrypt_block(Iter block) const
{
...
template<typename Iter>
void AES::inverse_sub_bytes(Iter block) const
{
	for (int i = 0; i < BLOCK_SIZE; i++)
	{
...
template<typename Iter>
void AES::inverse_shift_rows(Iter block) const
{
	for (int i = 1; i < WORD_SIZE; i++)
	{
		word row;
...
template<typename Iter>
void AES::inverse_mix_columns(Iter block) const
{
	state mixed;
